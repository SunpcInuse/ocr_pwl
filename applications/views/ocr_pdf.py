...
from applications.functions.report_pdf2 import RLPDF2


@demo_bp.route('/pdf', methods=['GET'])
def url_ocr_pdf():
    if request.method == "GET":
        return render_template("pdf_demo.html")
...
def url_pic_upload():
    ...
    file_path_list = sort_file_name(save_path_list)
    ocr_to_pdf = RLPDF2(filenamePrefix)
    ocr_to_pdf.create_pdf_from_images(file_path_list)
    return jsonify({'message': 'Files successfully uploaded'}), 200


# download
@demo_bp.route('/download', methods=['GET'])
def download_pdf():
    # 指定PDF文件的路径
    if request.method == "GET":
        # print(request)
